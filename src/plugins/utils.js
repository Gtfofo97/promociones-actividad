...
Vue.prototype.isNit = (nit) => {
  ...
  if (nit.length === 17) {
    // Validando que no sea ceros todos los dígitos
    if (
      nit !== '0000-000000-000-0' && nit.match(/^[0-9]{4}-[0-9]{6}-[0-9]{3}-[0-9]$/)
    ) {
      ...
      if (correlativo <= 100) {
        // Obteniendo la suma corresponiente
        suma = digitos.reduce(
          (total, digito, index) => (total += parseInt(digito, 10) * (14 - index)),
          0,
        );
        ...
        // Obteniendo la suma corresponiente
        suma = digitos.reduce(
          (total, digito, index) => (total += parseInt(digito, 10) * parseInt(3 + 6 * Math.floor((index + 5) / 6) - (index + 1), 10)),
          0,
        );
        // Obteniendo el Modulo base 11
        mod = suma % 11;
...
const buscarRuta = (rutas, ruta) => rutas.some((item) => item.nombre_uri.toLowerCase() === ruta.name.toLowerCase());
...
Vue.prototype.canNext = async (ruta) => {
  ...
  if (permit.some((row) => row.toLowerCase() === ruta.name.toLowerCase())) return true;
